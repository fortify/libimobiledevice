    ...
    const char *src_path)
{
    ...
    if (afcrc != AFC_E_SUCCESS) {
        if (afcrc == AFC_E_PERM_DENIED) {
            printf("Permission denied when getting info for '%s'.\n", src_path);
        } else {
            printf("Error getting info for '%s': %d\n", src_path, afcrc);
        }
        goto cleanup;
    }
    if (stbuf.st_mode == S_IFDIR) {
        ...
        if (afcrc != AFC_E_SUCCESS) {
            if (afcrc == AFC_E_PERM_DENIED) {
                printf("Permission denied when getting directory listing for "
                    "'%s'.\n", src_path);
            } else {
                printf("Error getting directory listing for '%s'\n:  %d\n", 
                    src_path, afcrc);
            }
            goto cleanup;
        }
        ...
        printf("Skipping '%s' not a supported file: mode %d\n", src_path,
            stbuf.st_mode);
    }
cleanup:
    if (dirlist) {
...
int main(int argc, char **argv)
{
    ...
    if ( harc != HOUSE_ARREST_E_SUCCESS) {
        printf("Could not get result house arrest.  Error: %d\n", harc);
        rc = -1;
        goto cleanup;
    }
